...
export const PACK_ADMIN_COOKIE = 'boredroom_pack_admin';
export const PACK_ADMIN_TTL_MS = 8 * 60 * 60 * 1000;
...
export function verifyPackAdminPassphrase(passphrase: string, secret: string | undefined): boolean {
  return Boolean(secret && passphrase && safeEqual(passphrase, secret));
}

export function issuePackAdminSession(secret: string, now = Date.now()): string {
  const payload = Buffer.from(JSON.stringify({ exp: now + PACK_ADMIN_TTL_MS })).toString('base64url');
  return `${payload}.${signature(payload, secret)}`;
}

export function verifyPackAdminSession(
...
export function isAllowedPackAdminOrigin(origin: string | undefined): boolean {
  if (!origin) return true;
  const configured = (process.env.PACK_ADMIN_ORIGINS ?? 'https://party.hendrix.com.ng')
    .split(',')
