...
if [[ "${DRY_RUN:-0}" == "1" ]]; then
  ...
  echo "[deploy] Would run docker compose build, container restart, and health checks"
  exit 0
fi
...
ssh "$TARGET_HOST" "cd '$TARGET_DIR' && for i in \$(seq 1 30); do if curl -fsS http://127.0.0.1:2567/healthz >/dev/null 2>&1; then exit 0; fi; sleep 2; done; echo '[deploy] health check failed after 60s' >&2; docker compose ps >&2; docker compose logs --tail=120 server >&2; exit 1"
