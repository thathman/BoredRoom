...
Last updated: 2026-06-27 09:50 WAT

> **Codex release completion:** BoredRoom `1.5.0.0` is deployed on Dell from main commit `3044e4f`. BoredRoom-Games `v1.3.0.1` is published and all 15 games are installed on Dell at `1.3.0.1`; catalog reports no updates. `v1.3.0.0` is superseded because its generated server import path was unloadable. The release verifier now proves extracted entrypoint loadability, runtime construction, digest, and Ed25519 signature. GitHub Actions release run: `28284189103`. Live passes: health, entry, permissions, votes, bots, consecutive games/reconnect, and expanded 15-game matrix. Post-deploy E2E determinism fixes and this evidence are committed with the final handoff. Physical-device microphone and QR-camera evidence is still pending broader QA.

> **Codex continuation (four-game completion checkpoint):** The user required Landlord auctions/trades, real Word Wahala board placement, Pidgin microphone/transcription, and Faith Feud survey/buzzer flow before release work. All four are implemented with dedicated runtime/UI tests and are now released/deployed as described above. Do not use local `.signing/private.pem`; it is ignored, mode 600, and does not match the production public key. Production signing remains in the GitHub Actions secret.
