...
Last updated: 2026-07-01 (Claude — Money Trivia hardened + RELEASED live)

> **Claude — Money Trivia hardening + release (2026-07-01, app 1.10.0.0 / games 1.7.0.0 LIVE):** Closed every verified pre-release gap and shipped. **Security:** a generic `publicGameRun` projector strips questions/answers/AI content from every `session:state` + REST response (regression-tested — no answer text/index reaches any projection). **Runtime:** dedicated Fastest Finger ordering questions (15+/age band, real correct-order metadata); full deadline auto-resolution — the runtime reports `nextDeadline()` and `resolveDueDeadlines()`, the server schedules a resolver for FF expiry / 4 s auto-reveal / question timeout / lifeline expiry; lifelines pause the question timer and resume with ≥15 s; game pause/resume shifts all deadlines (fake-clock tested; games suite 253). **Persistence:** migration adds `game_runs.result` + a server-only `money_trivia_questions` table (RLS on); results/settlement persist and survive restart; question CRUD 503s without durable persistence; startup hydrates the DB over the seed. **Content:** audited the 225-seed bank (fixed malformed/ambiguous/stale items), strict category enforcement (blocks setup if categories can't fill all 15 levels), content-quality regression guard. **UX:** index-0 reveal fix, hidden Rounds/Pace, explicit Ask-a-Player selector, helper/host confidence, persistent recap payout, original synthesized audio (no reused Feud/Hustle cues), owner review UI in /games with real AI draft generation (drafts unverified until approved). **CI:** cross-repo recovery via a Vite-ignored dynamic import + games checkout; repaired the long-red app CI (game-admin token, real smoke scripts, Money Trivia excluded from the generic matrix) — **both repos green**. **Release:** tagged games `v1.7.0.0` → Actions signed & published all 15 artifacts + catalog (Money Trivia 💰 verified); deployed app 1.10.0.0 to Dell; reconciled all 15 installed games to 1.7.0.0; applied the migration to Dell Postgres; health green. Also fixed a Dell outage mid-deploy: Tailscale's MagicDNS was SERVFAIL'ing all public DNS — **disconnected Tailscale (per Hendrix) and restored resolv.conf to 1.1.1.1/8.8.8.8** — and hardened the server so a catalog-fetch failure logs instead of crash-looping. **Left for Hendrix:** the live Money Trivia end-to-end playtest (host + companion + ≥4 controllers through fastest finger/ties/all-lifelines/all-outcomes/reconnect/recap settlement).

Last updated (prior): 2026-06-30 (Claude — Money Trivia conversion, code-complete)

> **Claude — Money Trivia (replaces Who Sabi Pass; id stays `trivia`) — 2026-06-30, app 1.10.0.0 / games manifest 1.7.0.0:** Built the full game end-to-end (live playtesting reserved for the user). **Runtime** (`BoredRoom-Games/runtime/games/money-trivia.js`): synchronized Fastest Finger (server-time ranking, 100 ms tie-break, reload on all-wrong) → one contestant → 15-question hot-seat cash ladder; canonical ₦100→₦5,000 ladder + arbitrary `start+(top−start)(i/14)^2.1` generator (nice-rounded, strictly increasing, exact endpoints); select/lock(Final answer)/host-reveal+4 s fallback/advance; configurable safety nets, walk-away, timeout outcome; all four lifelines (50:50, Ask the Room anonymous tally, Ask a Player, Ask the Host) with the correct answer withheld until reveal; no bots; result object (contestant/pledged/earned/outcome/currency/unsettled). 21 runtime tests incl. server-time/replay/vote-gating security; games suite **248 pass**. **Content** (`server/src/content/`): 225-question approved seed bank (3 age bands × 15 levels × 5, Nigerian-weighted, each with category/difficulty/explanation/source/review status+date/optional expiry) + store with validation (malformed/unsourced/wrong-tier/duplicate/expiry), draft→approve workflow, run-selection that **blocks setup** when a level lacks approved content. **Server**: trivia start injects only approved questions, forces `aiContent:false`, requires host-funded confirmation; `GameRun.result` + `finishActiveGame` capture; `session:mark_payout` (host-auth); owner GET/POST/PATCH/DELETE `/games/trivia/questions`. **UI**: dedicated `MoneyTriviaSurface` (emerald/purple neon, gold prize accents) for display/controller/companion across both phases + payout settle; config sheet age band/prizes/timers/timeout/lifelines + required host-funded gate. 14 content + 3 surface tests; web suite **158 pass**. Gates green (web typecheck/lint/build, server build, both test suites). Pushed both repos. **Catalog build verified** (15 signed artifacts bundle with an ephemeral key). **BLOCKED for live: the official signed games release (1.7.0.0) needs the maintainer-held `BOREDROOM_GAMES_SIGNING_KEY`** — until it is re-signed and the Dell catalog reconciled, the installed `trivia` artifact stays the old Who Sabi Pass runtime, so the app is intentionally NOT deployed to avoid a half-state. To finish live: run the signed release with the key, reconcile Dell, then deploy app 1.10.0.0.

Last updated (prior): 2026-06-27 20:45 WAT (Claude artifact-rejection tests — CUR-05 byte-level gate)

> **Claude continuation (artifact-rejection tests — 2026-06-27):** Closed the byte-level half of the CUR-05 "game artifact rejection" gap. Extracted the install gate's size/digest/signature checks from `installGame` into a pure, exported `verifyArtifactBytes(bytes, expected, publicKeyPem)` (behaviour-preserving refactor — `installGame` now calls it), and added `src/test/artifactRejection.test.ts` covering the rejection paths with a freshly generated Ed25519 keypair signing over the hex digest exactly like the release pipeline: oversized → `artifact_too_large`, length mismatch → `artifact_size_mismatch`, tampered same-length bytes → `artifact_digest_invalid`, and a signature forged by a different key → `artifact_signature_invalid`, plus the happy path returning the digest. App suite 134 passed + 2 skipped (was 129). Gates green: web typecheck/lint/build, server build. Deployed to Dell; `/healthz` green. (Still open in CUR-05: archive-level rejection — path traversal, symlink/hardlink, bad MIME, manifest mismatch — those checks exist in `installGame` but need a fixture tarball harness to assert; the tar/path guards are already enforced at `installedGames.ts:317-335`.)

Last updated (prior): 2026-06-27 20:30 WAT (Claude per-game config screen + fast-paced round timer)

> **Claude continuation (per-game configuration screen + fast-paced timer — 2026-06-27, BoredRoom 1.7.0.0):** Closed the GOAL1 "every game has a pre-game configuration screen" line. New `GameConfigSheet` opens for **every** game before it starts (rounds 3/5/7/10, pace Relaxed/Normal/Fast/Blitz, bot count for bot-capable games, hints toggle); `SessionScreen.chooseGame` now opens the config sheet instead of auto-starting, and `startConfigured` locks the chosen settings into the run. The pace setting drives a new **server-authoritative pace timer** in `HouseSessionRoom`: when a round drags it force-reveals after `timerMs` (Normal 20s / Fast 12s / Blitz 7s) then auto-advances after a short reveal countdown (3–5s); Relaxed leaves games untimed and turn-based phases are untouched. Timer is cleared on dispose. The pace timer now broadcasts its `paceDeadline`, and every game surface (display + controller) renders a shrinking neon **round countdown bar** with a seconds readout that turns red in the final 4s — so the auto-advance is visible, not a surprise (closes the "no browser proof of timer UI" gap). The config sheet now also exposes **game-specific rule settings** for every game — but only keys the deployed runtimes actually read (audited from `BoredRoom-Games/runtime/games/*.js`): Whot special-cards/reverse, ETTT marks/target/teams, Connect 4 best-of/teams, Faith Feud steals/AI-surveys, Hustle dice/density/quick, Landlord cash/quick, Logo reveal-stages, Market tolerance, Pidgin mode/direction, Color difficulty, Word rack-size, Trivia/Bible AI-content. No placeholder knobs. This closes GOAL1 section E (universal + game-specific pre-game settings) for all 15 games. Re-deployed; live 15-game matrix PASS on first run. Gates green: web typecheck/lint/build, server build, 128 app tests pass + 2 skipped (incl. new `gameConfigSheet.test.tsx`). Deployed HEAD `52815ed` to Dell; live 15-game matrix PASS (one whot run hit the pre-existing deck RNG flake, clean on rerun), live `/tts` returns a 22KB Naija MP3. Committed as Hendrix Nwaokolo, pushed to main. Remaining GOAL1/GOAL2 is the long tail: per-game game-specific config extras, browser E2E (reconnect/server-restart/vote UI), visual-regression ledger, and the signed-artifact release — still **blocked on the maintainer-held `BOREDROOM_GAMES_SIGNING_KEY`** and physical-device QR/camera evidence.

Last updated (prior): 2026-06-27 11:35 WAT (Codex host/controller regression release)
...
| CUR-05 | Partially met | Game admin auth exists. **Byte-level artifact rejection now tested** (`artifactRejection.test.ts`: oversized/size/digest/forged-signature). Still need archive-level rejection fixtures (path traversal, symlink/hardlink, bad MIME, manifest mismatch — guards exist), install/update/uninstall auth, origin validation, active-run blocking, expired/altered cookies. |
